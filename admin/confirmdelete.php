...
$db_vacidno = $_REQUEST["vacidno"];

include("connections.php");

$query_delete = mysqli_query($connections, "SELECT * FROM tbl_vacinfo WHERE vacidno = '$db_vacidno' ");

	while($row_delete = mysqli_fetch_assoc($query_delete)){
		$user_id = $row_delete["id"];
		$db_vacidno = $row_delete["vacidno"];
		...
		$db_contactperson = $row_delete["contactperson"];
		$db_relation = $row_delete["relation"];
		$db_relcontactnumber = $row_delete["relcontactnumber"];
		$db_category = $row_delete["category"];		
		
	}
	
	echo  "<p style='text-align: left;' class='col-md-8 fs-4'> Are you sure you want to delete <br> $db_vacidno - $db_firstname $db_middlename $db_surname $db_suffix ? </p>";
?>				
				
	<form method="POST" action="Delete_Now1.php" autocomplete="on">

		<input type="hidden" name="db_vacidno" value="<?php echo $db_vacidno; ?>">
