		...
		$fb_email = $fb_message =  "";
			...
			if ($_SERVER["REQUEST_METHOD"] == "POST"){
				...
				if ($fb_email && $fb_message) {
					...
					echo "<script>window.location.href='firstdetails.php';</script>";
				}
			}
?>
                ...
                <div class="form-group mb-3">
                    <div id="formdiv">
                        <div class="row" style="margin-right:0px;margin-left:0px;padding-top:24px;">
                            <div class="col">
                                  <h4 style="border-color: #009933;color: rgb(254,142,23);font-size: 32px;text-align: center;"><strong>VACCINATION DETAILS</strong><br></h4>
                              
                            </div>                         
                        </div>
                      
                        <div class="row" style="margin-right:0px;margin-left:0px;padding-top:24px;">
                            <div class="col-12 col-md-4 offset-md-4 d-flex align-items-center align-content-center" style="width: 252.646px;height: 77px;"><a class="btn btn-light btn-lg" role="button" style="font-family: Roboto, sans-serif;background: #fe8e17;font-size: 14px;color: rgb(255,255,255);width: 95.1562px;" href="firstdosepre.php">PRE-VAC</a><a class="btn btn-light btn-lg" role="button" style="margin-left: 16px;color: rgb(255,255,255);background: #fe8e17;font-size: 14px;width: 104.0625px;font-family: Roboto, sans-serif;" href="firstdosepost.php">POST-VAC</a></div>
                        </div>
                    </div>
                </div>
            </form>
...
$sql = "SELECT * FROM tbl_vacdetails1stdose";
...
$view_query = mysqli_query($connections, "SELECT * FROM tbl_vacdetails1stdose ORDER BY id ASC ");
...
while ($row = mysqli_fetch_assoc($view_query)) {
	...
	$db_name = $row["name"];
	$db_dosage_sequence = $row["dosage_sequence"];
	$db_vac_site = $row["vac_site"];
	$db_vac_brand = $row["vac_brand"];
	$db_batch_number = $row["batch_number"];
	$db_lot_number = $row["lot_number"];
	$db_date_vaccination = $row["date_vaccination"];
	$db_vaccinator = $row["vaccinator"];
	...
	<tr>
		<td>$db_vacidno</td>
		<td>$db_name</td>
		<td>$db_dosage_sequence</td>
		<td>$db_vac_site</td>
		<td>$db_vac_brand</td>
		<td>$db_batch_number</td>
		<td>$db_lot_number</td>
		<td>$db_date_vaccination</td>
		<td>$db_vaccinator</td>
		<td>$db_filename</td>
		<td>
		<a href='editdetails1.php?id=$user_id' style='text-decoration:none; color:#009933;'>Update</a>
		</td>
	</tr>
